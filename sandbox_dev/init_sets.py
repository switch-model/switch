...
def init_DISPATCH_SCENARIOS_IN_PERIOD (model, period):
  return (s for s in model.DISPATCH_SCENARIO if model.dispatch_scenario_period[s] == period)
model.DISPATCH_SCENARIOS_IN_PERIOD = Set(model.INVEST_PERIODS, within=model.DISPATCH_SCENARIO, \
  initialize=init_DISPATCH_SCENARIOS_IN_PERIOD)

def init_SET_FROM_PARAM (model):
  return set(model.dispatch_scenario_period[s] for s in model.DISPATCH_SCENARIO)
