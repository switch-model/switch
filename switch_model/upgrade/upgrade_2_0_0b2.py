...
"""
Upgrade input directories from 2.0.0b0 to 2.0.0b2.
Major changes are:
...
# whitelist of old modules;
# if one of these is referenced without the switch_mod prefix,
# it will be treated as a synonym.
old_modules = {
...
rename_modules = {
    'switch_mod.load_zones': 'switch_model.balancing.load_zones',
    'switch_mod.fuels': 'switch_model.energy_sources.properties',
    'switch_mod.trans_build': 'switch_model.transmission.transport.build',
    'switch_mod.trans_dispatch': 'switch_model.transmission.transport.dispatch',
    'switch_mod.project.build': 'switch_model.generators.core.build',
    'switch_mod.project.discrete_build': 'switch_model.generators.core.proj_discrete_build',
    'switch_mod.project.dispatch': 'switch_model.generators.core.dispatch',
    'switch_mod.project.no_commit': 'switch_model.generators.core.no_commit',
    'switch_mod.project.unitcommit.commit': 'switch_model.generators.core.commit.operate',
    'switch_mod.project.unitcommit.fuel_use': 'switch_model.generators.core.commit.fuel_use',
    'switch_mod.project.unitcommit.discrete': 'switch_model.generators.core.commit.discrete',
    'switch_mod.fuel_cost': 'switch_model.energy_sources.fuel_costs.simple',
    'switch_mod.fuel_markets': 'switch_model.energy_sources.fuel_costs.markets',
    'switch_mod.export': 'switch_model.reporting',
    'switch_mod.local_td': 'switch_model.transmission.local_td',
    'switch_mod.balancing_areas': 'switch_model.balancing.operating_reserves.areas',
    'switch_mod.export.dump': 'switch_model.reporting.dump',
    'switch_mod.generators.hydro_simple': 
...
# Build a master list of module name replacements.
# This includes the simple renames, the expansions, and 
# any modules previously written without the switch_mod prefix.
replace_modules = expand_modules.copy()
replace_modules.update({k: [v] for (k, v) in rename_modules.items()})
for m in old_modules:
    # find abbreviated name of old module, which may have been used in modules.txt
    abbrev = '.'.join(m.split('.')[1:])
    if m in replace_modules:
        replace_modules[abbrev] = replace_modules[m]
    else:
        # make sure every module name now starts with switch_model
        # (not omitted or switch_mod)
        replace_modules[m] = ['switch_model.'+abbrev]
        replace_modules[abbrev] = ['switch_model.'+abbrev]
...
def upgrade_input_dir(inputs_dir, verbose=False, backup=True):
    ...
    for module in module_list:
        if module in replace_modules:
            new_module_list.extend(replace_modules[module])
        else:
