...
module_messages = {
    ...
    "switch_model.generators.extensions.hydro_system": (
        """
        The hydro_system module now tracks reservoir levels over the course of
        each timeseries instead of each period. For models with a single, long
        timeseries for each period, this won't make a difference. However, for
        models with multiple timeseries in each period with different weights,
        this fixes a problem where the hydro module treated timepoints in
        different timeseries as if they were sequenced with each other, so it
        could store water in a low-weight timeseries and use it in high-weight
        timeseries, creating free energy. Although this approach is more correct
        than before, it is also more conservative: hydro networks must reach
        the same level (or a prespecified level) at the start and end of each
        ...
        * Users should now set res_initial_vol and res_final_vol for each
        ...
        and final_res_vol for each period in reservoirs.csv.

        * res_initial_vol and res_final_vol are now optional. If
        res_initial_vol is set but not res_final_vol, Switch will set the final
        level equal to the initial level. If neither is set, Switch will choose
        an optimal initial volume for each timeseries and also return to that
        level at the end of the timeseries.
        """
    )
}
...
def upgrade_input_dir(inputs_dir):
    ...
    # rename modules (not needed in this version) and report changes
    update_modules(inputs_dir)

    # move the initial and final reservoir volumns from reservoirs.csv to
    # reservoir_ts_data.csv
    move_hydro_initial_final_res_vol(inputs_dir)
...
# move the initial and final reservoir volumns from reservoirs.csv to
# reservoir_ts_data.csv
def move_hydro_initial_final_res_vol(inputs_dir):
    res_file = os.path.join(inputs_dir, "reservoirs.csv")
    res_ts_file = os.path.join(inputs_dir, "reservoir_ts_data.csv")
    if os.path.exists(res_file):
        res = pd.read_csv(res_file, na_values=".")
        interp = pd.read_csv(os.path.join(inputs_dir, "timeseries.csv"))
        interp["dur"] = interp.eval(
            "ts_duration_of_tp * ts_num_tps * ts_scale_to_period"
        )
        interp["start_frac"] = interp.groupby("ts_period")["dur"].transform(
            lambda x: (x.cumsum() - x) / x.sum()
        )
        interp["end_frac"] = interp.groupby("ts_period")["dur"].transform(
            lambda x: x.cumsum() / x.sum()
        )
        interp["dummy"] = 1  # used for cross-join later
        res_ts = res.assign(dummy=1).merge(interp, on="dummy").drop(columns=["dummy"])
        # interpolate the starting and ending values across the timeseries
        res_ts["res_initial_vol"] = res_ts.eval(
            "(1 - start_frac) * initial_res_vol + start_frac * final_res_vol"
        )
        res_ts["res_final_vol"] = res_ts.eval(
            "(1 - end_frac) * initial_res_vol + end_frac * final_res_vol"
        )

        # choose the right columns and save the input files
        res_ts = res_ts[
            ["RESERVOIRS", "TIMESERIES", "res_initial_vol", "res_final_vol"]
        ]
        res_ts.to_csv(res_ts_file, index=False, na_rep=".")

        res = res.drop(columns=["initial_res_vol", "final_res_vol"])
        res.to_csv(res_file, index=False, na_rep=".")

        switch_model.upgrade.print_verbose()
        switch_model.upgrade.print_verbose(
            """
            Moved initial_res_vol and final_res_vol from reservoirs.csv to
            res_initial_vol and res_final_vol in reservoir_ts_data.csv.
            """
        )
        if interp["TIMESERIES"].nunique() > interp["ts_period"].nunique():
            switch_model.upgrade.print_verbose()
            switch_model.upgrade.print_verbose(
                """
                WARNING: Intermediate values were assigned to res_final_vol and
                res_initial_vol for timeseries boundaries within each period.
                These may change model results.
                """
            )
        switch_model.upgrade.print_verbose()
