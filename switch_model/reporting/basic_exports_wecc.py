...
#        
#        n_scen = 1# mod.SCENARIOS.__len__()
...
#            gen_projects[g] = []
#            for prj in mod.GENERATION_PROJECTS:
#                if mod.gen_tech[prj]==g:
...
#
        ...
        table = export.write_table(
            mod, mod.GENERATION_TECHNOLOGIES,
            output_file=os.path.join(summaries_dir, table_name+".csv"),
            headings=('gen_tech', 'interest_rate' )+ tuple(p 
                for p in mod.PERIODS),
            values=lambda m, gt: (gt, m.interest_rate) + tuple( sum(m.DispatchGen[g, t] 
                for g in m.GENERATION_PROJECTS
                if m.gen_tech[g] == gt for t in m.TPS_IN_PERIOD[pp]) for pp in m.PERIODS))
