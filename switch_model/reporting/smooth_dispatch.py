...
def define_dynamic_components(m):
    ...
    def rule(m):
        ...
        if len(smoothing_comps) == 0:
            ...
            m.logger.info(
                wrap(
                    f"Will smooth {', '.join(smoothing_comps[:-2])} and {smoothing_comps[-1]}."
                )
            )
...
def post_iterate(m):
    ...
    if m.iteration_number == 0:
        ...
        raise RuntimeError(
            f"Reached unexpected iteration number {m.iteration_number} in module {__name__}."
        )
...
def post_solve(m, outputs_dir):
    ...
    if not getattr(m, "iterated_smooth_dispatch", False):
        pre_smooth_solve(m)
