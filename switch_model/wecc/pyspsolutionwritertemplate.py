...
def write_csv_soln(scenario_tree, output_file_prefix):
    ...
    if isinstance(scenario_tree, ScenarioTree):
        print("Yay.")
        for scenario_instance in scenario_tree._scenarios:
            switch_model.utilities.post_solve(
                scenario_instance._instance,
                outputs_dir=os.path.join("outputs", scenario_instance.name),
            )
    ...
    with open(cost_filename, "w") as f:
        for stage in scenario_tree.stages:
            ...
            for tree_node in sorted(stage.nodes, key=lambda x: x.name):
                for scenario in sorted(tree_node.scenarios, key=lambda x: x.name):
