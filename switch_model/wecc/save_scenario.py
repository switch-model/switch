...
def main():
    ...
    ordered_params = list(filter(lambda v: v[1] is not None, scenario_params.__dict__.items()))
    columns = ','.join(v[0] for v in ordered_params)
    values = ','.join(f"'{v[1]}'" if type(v[1]) == str else str(v[1]) for v in ordered_params)
