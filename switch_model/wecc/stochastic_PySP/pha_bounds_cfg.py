...
def pysp_boundsetter_callback(self, scenario_tree, scenario):
    ...
    # BuildLocalTD # Paty commented this block because we are not including LocalTD in the modules
    ...
    system_wide_peak = {}
    for p in m.PERIODS:
        system_wide_peak[p] = sum(m.lz_peak_demand_mw[lz, p] for lz in m.LOAD_ZONES)

    # BuildProj
    for proj, bld_yr in m.PROJECT_BUILDYEARS - m.EXISTING_PROJ_BUILDYEARS:
        if proj not in m.PROJECTS_CAP_LIMITED:
            m.BuildGen[proj, bld_yr].setub(5 * system_wide_peak[bld_yr])
