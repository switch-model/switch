...
def define_components(mod):
    ...
    mod.zone_total_generation_in_period_mwh = Expression(
        mod.LOAD_ZONES, mod.PERIODS,
        rule=lambda m, z, p: (
            sum(m.DispatchGen[g, t] * m.tp_weight[t]
    ...
    mod.RPS_Enforce_Target = Constraint(
        ...
        rule=lambda m, p: (m.RPSFuelEnergy[p] + m.RPSNonFuelEnergy[p] >=
            sum(m.rps_target[z, p] * m.zone_total_generation_in_period_mwh[z, p] for z in m.LOAD_ZONES)))
...
def post_solve(instance, outdir):
    ...
    write_table(
        ...
        values=lambda m, p: (p, m.RPSFuelEnergy[p] / 1000,
                                m.RPSNonFuelEnergy[p] / 1000,
        						total_generation_in_period(m,p) / 1000,
        						(m.RPSFuelEnergy[p] + m.RPSNonFuelEnergy[p]) / total_generation_in_period(m,p),
