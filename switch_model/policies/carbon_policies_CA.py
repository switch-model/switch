...
def define_components(model):
    ...
    model.carbon_cap_tco2_per_yr_CA = Param(model.PERIODS, default=float('inf'), doc=(
        "Emissions from this model must be less than this cap. "
        "This is specified in metric tonnes of CO2 per year."))        
    model.AnnualEmissions_CA = Expression(model.PERIODS,
    ...
    model.Enforce_Carbon_Cap_CA = Constraint(model.PERIODS,
...
def load_inputs(model, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        auto_select=True,
        param=(model.carbon_cap_tco2_per_yr, model.carbon_cap_tco2_per_yr_CA, model.carbon_cost_dollar_per_tco2))
...
def post_solve(model, outdir):
    ...
    reporting.write_table(
        ...
        headings=("PERIOD", "AnnualEmissions_tCO2_per_yr", 
                  "carbon_cap_tco2_per_yr", "AnnualEmissions_tCO2_per_yr_CA", 
