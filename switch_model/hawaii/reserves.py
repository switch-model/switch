...
def define_components(m):
    ...
    m.FIRM_GEN_TPS = Set(
        initialize=m.GEN_TPS, filter=lambda m, p, tp: p in m.FIRM_GENECTS
    )
    ...
    m.CONTINGENCY_GEN_TPS = Set(
        initialize=m.GEN_TPS, filter=lambda m, p, tp: p in m.CONTINGENCY_GENECTS
    )
    ...
    m.regulating_reserve_requirement_mw = Expression(
        m.TIMEPOINTS,
    ...
    m.SpinningReserveUpRequirement = Expression(
        m.TIMEPOINTS,
        rule=lambda m, tp: m.regulating_reserve_requirement_mw[tp]
        + m.ContingencyReserveUpRequirement[tp],
    )
    # require 10% down reserves at all times
    m.SpinningReserveDownRequirement = Expression(
        m.TIMEPOINTS,
        rule=lambda m, tp: 0.10 * sum(m.zone_demand_mw[z, tp] for z in m.LOAD_ZONES),
    )
...
def define_dynamic_components(m):
    ...
    # Available reserves
    m.SpinningReservesUpAvailable = Expression(
        m.TIMEPOINTS,
        rule=lambda m, tp: sum(
            m.DispatchSlackUp[p, tp] for p in m.FIRM_GENECTS if (p, tp) in m.GEN_TPS
        )
        + (
    ...
    m.SpinningReservesDownAvailable = Expression(
        m.TIMEPOINTS,
        rule=lambda m, tp: sum(
            m.DispatchSlackDown[p, tp] for p in m.FIRM_GENECTS if (p, tp) in m.GEN_TPS
        )
        + (
