...
def define_components(m):
    ...
    m.CURRENT_AND_PRIOR_PERIODS = Set(
        m.PERIODS,
        ordered=True,
    ...
    m.GENERATION_PROJECTS_BY_FUEL = Set(
        m.FUELS,
        initialize=lambda m, f: sorted(
            [p for p in m.FUEL_BASED_GENS if f in m.FUELS_FOR_GEN[p]]
        ),
    )
    m.GENERATION_PROJECTS_BY_NON_FUEL_ENERGY_SOURCE = Set(
        m.NON_FUEL_ENERGY_SOURCES,
        initialize=lambda m, s: sorted(
            [p for p in m.NON_FUEL_BASED_GENS if m.gen_energy_source[p] == s]
        ),
    )
