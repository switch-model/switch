...
def write_results(m, outputs_dir):
    ...
    battery_capacity_mw = lambda m, z, pe: (
        (
            m.Battery_Capacity[z, pe]
            * m.battery_max_discharge
            / m.battery_min_discharge_time
        )
        if hasattr(m, "Battery_Capacity")
        else 0.0
    )
    ...
    util.write_table(
        ...
        output_file=os.path.join(
            outputs_dir, "capacity_by_technology{t}.tsv".format(t=tag)
        ),
        headings=("load_zone", "period")
        + built_tech
        + ("hydro", "batteries", "fuel cells"),
    ...
    util.write_table(
        ...
        m.PERIODS,
        output_file=os.path.join(
            outputs_dir, "capacity_by_energy_source{t}.tsv".format(t=tag)
        ),
        headings=("load_zone", "period")
        + built_energy_source
        + ("hydro", "batteries", "fuel cells"),
    ...
    def cost_breakdown_details(m, z, pe):
        ...
        values += [
            sum(
                ...
                for g in built_gens
                if m.gen_tech[g] == t
                and m.gen_load_zone[g] == z
                and (g, pe) in m.BuildGen
            )
            for t in built_tech
        ]
        ...
        values.append(
            sum(
                m.BuildPumpedHydroMW[g, pe] for g in m.PH_GENS if m.ph_load_zone[g] == z
            )
            if hasattr(m, "BuildPumpedHydroMW")
            else 0.0,
        )
        ...
        values += [
            sum(
                m.BuildGen[g, pe]
                * (m.gen_overnight_cost[g, pe] + m.gen_connect_cost_per_mw[g])
                for g in built_gens
                if m.gen_tech[g] == t
                and m.gen_load_zone[g] == z
        ...
        if hasattr(m, "battery_capital_cost_per_mwh_capacity"):
            ...
            values.append(
                m.BuildBattery[z, pe] * m.battery_capital_cost_per_mwh_capacity
            )
        elif hasattr(m, "battery_capital_cost_per_mwh_capacity_by_year"):
            values.append(
                m.BuildBattery[z, pe]
                * m.battery_capital_cost_per_mwh_capacity_by_year[pe]
            )
        else:
            values.append(0.0)
        # hydro
            ...
            values.extend(
                [
                    sum(
                        ...
                        for rfm_st in m.SUPPLY_TIERS_FOR_RFM_PERIOD[rfm, pe]
                    )
        ...
        values.append(
            annualize_present_value_period_cost(m, pe, m.SystemCostPerPeriod[pe])
        )
    ...
    util.write_table(
        ...
        + (
            "h2_electrolyzer_overnight_cost",
            "h2_liquifier_overnight_cost",
            "liquid_h2_tank_overnight_cost",
            "fuel_cell_overnight_cost",
        )
        + (
        ...
        + (("ev_extra_capital_recovery",) if hasattr(m, "ev_extra_annual_cost") else ())
