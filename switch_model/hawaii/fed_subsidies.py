...
def define_components(m):
    ...
    m.Wind_Subsidy_Hourly = Expression(
        m.TIMEPOINTS,
        rule=lambda m, t: -wind_prod_tax_credit
        * sum(
            m.DispatchGen[p, t]
    ...
    m.Solar_Credit_Annual = Expression(
        m.PERIODS,
        rule=lambda m, pe: -solar_invest_tax_credit
        * sum(
            m.BuildGen[g, bld_yr] * m.gen_capital_cost_annual[g, bld_yr]
            for g in m.GENERATION_PROJECTS_BY_NON_FUEL_ENERGY_SOURCE[
                solar_energy_source
            ]
