...
class GraphTools:
    ...
    def get_energy_source_color_map(self, n):
        return {source: [color] * n for source, color in self.energy_source_color_map.items()}
    ...
    def get_new_axes(self, out, title=None, size=(8, 5)):
        ...
        if self.active_compare_dir == 0:
            fig, ax = plt.subplots(nrows=1, ncols=self.num_compares, sharey=True, sharex=True)
