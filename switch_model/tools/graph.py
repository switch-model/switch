...
class GraphData:
    ...
    def __init__(self):
        self.dfs = {}
        self.dir_mapping = {
            GraphDataFolder.OUTPUTS: "outputs",
            GraphDataFolder.INPUTS: "inputs",
        }

    def get_dataframe(self, csv, folder=GraphDataFolder.OUTPUTS):
        if csv not in self.dfs:
            self.dfs[csv] = pd.read_csv(
                os.path.join(self.dir_mapping[folder], csv + ".csv")
            )

        return self.dfs[
            csv
        ].copy()  # we return a copy so the source doesn't get modified
