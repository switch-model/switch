...
class GraphMapTools:
    ...
    def graph_squares(self, df, bins=(5, 8, 10, 15), cmap="viridis", ax=None, size=20, unit=" h"):
        """
        ...
        colors, legend_labels, legend_colors = self._bin_data(df["value"], bins,
                                                              lambda x: cmap_func(float(x / (num_bins - 1))))
        for i, row in df.iterrows():
            x, y = row["geometry"].x, row["geometry"].y
            ax.scatter(x, y, marker="s", s=size * 2, color="dimgray")  # Add a black border
            ax.scatter(x, y, marker="s", s=size, color=colors[i])
        ax.legend(
            title="Storage duration (h)",
            handles=[self._tools.plt.lines.Line2D([], [], color=c, marker="s", markersize=5, label=l, linestyle="None")
    ...
    def graph_transmission(self, df, cutoff, ax=None, legend=True, zorder=-50):
        ...
        self._plot_states(ax)
        df.plot(ax=ax, column="value", legend=legend, cmap="Reds", zorder=zorder, norm=colors.LogNorm(vmin=cutoff, vmax=df.value.max()))
        return ax
