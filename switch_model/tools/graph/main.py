...
class DataHandler:
    ...
    def get_dataframe(self, filename, folder=None, from_inputs=False, convert_dot_to_na=False, force_one_scenario=False,
                      drop_scenario_info=True, **kwargs):
        """
...
class GraphTools(DataHandler):
    ...
    def graph_map_pychart(self, df, max_size=2500):
        ...
        try:
            ...
            raise Exception("Could not find package 'geopandas'. If on Windows make sure you install it through conda.")

        # Read shape files
        try:
            wecc_lz = geopandas.read_file(self.get_file_path("maps/wecc_102009.shp", from_inputs=True), crs="ESRI:102009")
            wecc_states = geopandas.read_file(self.get_file_path("maps/wecc_states_4326.shp", from_inputs=True))
        except FileNotFoundError:
