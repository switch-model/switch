...
def define_components(mod):
    ...
    mod.vbat_charge_limit_mw = Param(
        mod.VIRTUAL_BATTERIES, mod.TIMEPOINTS,
        default = float('inf'),
    ...
    mod.VbatCharge = Var(
        mod.VIRTUAL_BATTERIES, mod.TIMEPOINTS,
        within=NonNegativeReals,
        bounds=lambda m, vb, t:
        (
           0.0,
           m.vbat_charge_limit_mw[vb,t]
           )
        )
    ...
    mod.VbatCumulativeCharge = Expression(
        mod.VIRTUAL_BATTERIES, mod.TIMEPOINTS,
        rule=lambda m, vb, t: \
            sum(m.VbatCharge[vb,tau]*m.tp_duration_hrs[tau]
    ...
    mod.Vbat_Cumulative_Charge_Upper_Limit =  Constraint(
        mod.VIRTUAL_BATTERIES, mod.TIMEPOINTS,
        rule=lambda m, vb, t:
        m.VbatCumulativeCharge[vb,t] <= m.vbat_cumulative_charge_upper_mwh[vb,t]) 

    mod.Vbat_Cumulative_Charge_Lower_Limit =  Constraint(
        mod.VIRTUAL_BATTERIES, mod.TIMEPOINTS,
        rule=lambda m, vb, t:
        m.VbatCumulativeCharge[vb,t] >= m.vbat_cumulative_charge_lower_mwh[vb,t]) 

    mod.ZoneTotalCharge = Expression(
        mod.LOAD_ZONES, mod.TIMEPOINTS,
        rule=lambda m, z, t: \
            sum(m.VbatCharge[vb, t]
                for vb in m.VBATS_IN_ZONE[z]))
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        autoselect=True,
        param=(mod.vbat_cumulative_charge_lower_mwh, mod.vbat_cumulative_charge_upper_mwh, mod.vbat_charge_limit_mw))
