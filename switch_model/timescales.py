...
def define_components(mod):
    ...
    * tp_weight[t] = 203.3 hr/period
    ...
    mod.period_start = Param(
        mod.PERIODS, within=NonNegativeReals, input_file="periods.csv"
    )
    mod.period_end = Param(
        mod.PERIODS, within=NonNegativeReals, input_file="periods.csv"
    )
    ...
    mod.ts_period = Param(
        mod.TIMESERIES, within=mod.PERIODS, input_file="timeseries.csv"
    )
    mod.ts_duration_of_tp = Param(
        mod.TIMESERIES, within=PositiveReals, input_file="timeseries.csv"
    )
    mod.ts_num_tps = Param(
        mod.TIMESERIES, within=PositiveIntegers, input_file="timeseries.csv"
    )
    mod.ts_scale_to_period = Param(
        mod.TIMESERIES, within=PositiveReals, input_file="timeseries.csv"
    )
    mod.min_data_check(
        "TIMESERIES",
    ...
    mod.tp_ts = Param(
        ...
        input_file="timepoints.csv",
        input_column="timeseries",
    )
    mod.min_data_check("TIMEPOINTS", "tp_ts")
    mod.tp_timestamp = Param(
        mod.TIMEPOINTS,
        default=lambda m, t: t,
        input_file="timepoints.csv",
