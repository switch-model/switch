...
def graph_state_of_charge(tools):
    ...
    pn = tools.pn
    plot = (
        pn.ggplot(df, pn.aes(x="datetime", y="charge"))
        + pn.geom_line()
        + pn.labs(
    ...
    tools.save_figure(
        "state_of_charge", by_scenario_and_time_row(tools, plot, num_rows).draw()
    )
...
def by_scenario_and_time_row(tools, plot, num_rows):
    ...
    return (
        plot
        + pn.facet_grid("time_row ~ scenario_name")
        + pn.theme(
            figure_size=(
                pn.options.figure_size[0] * tools.num_scenarios,
                pn.options.figure_size[1] * num_rows,
            )
        )
    )
