...
def define_components(mod):
    ...
    mod.hydro_ts_duration = Param(
        mod.HYDRO_TS,
        initialize=lambda m, hts: sum(
            m.tp_duration_hrs[tp] for tp in m.TPS_IN_HTS[hts]
        ),
    ...
    mod.Enforce_Hydro_Avg_Flow = Constraint(
        ...
        * sum(m.DispatchGen[g, t] * m.tp_duration_hrs[t] for t in m.TPS_IN_HTS[hts])
        / m.hydro_ts_duration[hts]
