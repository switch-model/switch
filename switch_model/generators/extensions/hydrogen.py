...
def define_hydrogen_components(m):
    ...
    m.StoreLiquidHydrogenKg = Expression(m.LOAD_ZONES, m.HGTS, rule=lambda m, z, hgts:
        m.hgts_duration_of_tp[hgts] * sum(m.LiquifyHydrogenKgPerHour[z, tp] for tp in m.TPS_IN_HGTS[hgts])
    )
    ...
    m.Max_Store_Liquid_Hydrogen = Constraint(
        ...
        m.HGP, 
        rule=lambda m, z, hgp:
        sum(m.StoreLiquidHydrogenKg[z, hgts] * m.hgts_scale_to_hgp[hgts] for hgts in m.HGTS_IN_HGP[hgp])
...
def post_solve(m, outdir):
    df = pd.DataFrame(
        {
            ...
            "total_capacity_liquifier_kg_hour": value(m.LiquifierCapacityKgPerHour[z,p]),
    ...
    write_table(
        m, output_file=os.path.join(outdir, "hydrogen_output_period.csv"), df=df, index=False
    )
    df = pd.DataFrame(
        {
            "hgtimeseries": hgts,
    ...
    write_table(
        m, output_file=os.path.join(outdir, "hydrogen_output_hgtimeseries.csv"), df=df, index=False
    )
    df = pd.DataFrame(
        {
            "timepoint": t,
            ...
            "electrolyzer_hydrogen_produced_kg_hour": value(m.ProduceHydrogenKgPerHour[z,t]),
            ...
            "fuelcell_hydrogen_consumed_kg_hour": value(m.ConsumeHydrogenKgPerHour[z,t]),
        }
        for z in m.LOAD_ZONES
        for t in m.TIMEPOINTS
    )
    write_table(
        m, output_file=os.path.join(outdir, "hydrogen_output_timepoint.csv"), df=df, index=False
    )
    df = pd.DataFrame(
        {
            "period": p,
